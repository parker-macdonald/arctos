mod about;
mod add_injury;
mod bracket;
mod docs;
mod edit_field;
mod edit_injury;
mod edit_match;
mod edit_player_profile;
mod edit_player_registration;
mod edit_tag;
mod edit_team_profile;
mod edit_team_registration;
mod finalize_match;
mod google_choose_account_type;
mod google_complete_profile;
mod index;
mod invitations;
mod layout;
mod license;
mod login;
mod manage;
mod match_page;
mod new_tournament;
mod player_profile;
mod players_list;
mod privacy;
mod record;
mod register;
mod results;
mod run_match;
mod schedule;
mod team_token_input;
mod scoreboard;
mod start_match;
mod stones;
mod team_profile;
mod teams_list;
mod terms;
mod thanks;
mod markdown_page;
mod tournament_home;
mod tournament_register;
mod tournament_setup;
mod tournament_settings;
mod bracket_setup;

pub use about::About;
pub use add_injury::AddInjury;
pub use bracket::Bracket;
pub use docs::Docs;
pub use edit_field::EditField;
pub use edit_injury::EditInjury;
pub use edit_match::EditMatch;
pub use edit_player_profile::EditPlayerProfile;
pub use edit_player_registration::EditPlayerRegistration;
pub use edit_tag::EditTag;
pub use edit_team_profile::EditTeamProfile;
pub use edit_team_registration::EditTeamRegistration;
pub use finalize_match::FinalizeMatch;
pub use google_choose_account_type::GoogleChooseAccountType;
pub use google_complete_profile::GoogleCompleteProfile;
pub use index::Index;
pub use invitations::Invitations;
pub use layout::Layout;
pub use license::License;
pub use login::Login;
pub use manage::Manage;
pub use match_page::{MatchPage, MatchPageById};
pub use new_tournament::NewTournament;
pub use player_profile::PlayerProfile;
pub use players_list::PlayersList;
pub use privacy::Privacy;
pub use record::Record;
pub use register::Register;
pub use results::Results;
pub use run_match::RunMatch;
pub use schedule::Schedule;
pub use team_token_input::TeamTokenInput;
pub use scoreboard::Scoreboard;
pub use start_match::StartMatch;
pub use stones::Stones;
pub use bracket_setup::BracketSetup;
pub use team_profile::TeamProfile;
pub use teams_list::TeamsList;
pub use terms::Terms;
pub use thanks::Thanks;
pub use markdown_page::MarkdownPage;
pub use tournament_home::TournamentHome;
pub use tournament_register::TournamentRegister;
pub use tournament_setup::TournamentSetup;
pub use tournament_settings::TournamentSettings;
